...
class Decoding:
    def numtoalp(self,str2):      #6430305622 24222462
        i=0
        str3=''                         #decoding on given string str2
        while i<len(str2):
            try:
              a=int(str2[i:i+2])
              if a>=22 and a<=72:
                str2=str2.replace(str(a),chr((((a*2+152)//4)+48)))      #'/' for float division and '//' for integer division
              elif a>=74 and a<=99:
                str2=str2.replace(str(a),chr(((((a-46)*2)-4)//2)+39))
